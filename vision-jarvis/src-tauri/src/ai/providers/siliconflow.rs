...
enum SFContent {
    ...
    ImageUrl { image_url: SFMediaUrl },
    #[serde(rename = "video_url")]
    VideoUrl { video_url: SFVideoUrl },
}
...
#[derive(Debug, Deserialize)]
struct SFChoice {
    message: SFResponseMessage,
}

#[derive(Debug, Deserialize)]
struct SFResponseMessage {
    content: String,
}
...
impl SiliconFlowProvider {
    ...
    async fn send_request(&self, messages: Vec<SFMessage>, model: &str) -> AppResult<String> {
        ...
        let ai_response: SFResponse = response.json().await
